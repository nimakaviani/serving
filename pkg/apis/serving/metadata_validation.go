...
func SetInitScaleAnnotation(ctx context.Context, resource interface{}) {
	...
	defaultConfig := config.FromContext(ctx).Defaults
	ans[InitScale] = strconv.FormatBool(defaultConfig.DisableDefaultReadinessOnDeploy)
}
